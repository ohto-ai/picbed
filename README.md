...
2. 修改根目录 `config.js` 里的 `worker` 为上面的地址（不填也行，首次打开页面会弹出设置框）；
   顺便把 `repo` 改成你的仓库地址，顶栏就会显示 GitHub 入口，留空则不显示
...
页面顶部（吸顶栏）有两个标签：**公开画廊**（默认，游客视角）与 **我的相册**（管理视角）。
本机浏览器里存过密码的话，打开页面会直接进「我的相册」；没有密码时点「我的相册」会引导你去设置里填。
顶栏右侧还有一个 **GitHub** 入口，指向 `config.js` 里的 `repo` 地址，**游客也能看到**——方便别人顺藤摸到源码；留空则不显示。

加载反馈：页面顶部有一条细进度条，图片网格在等待期间用带微光的**骨架卡片**占位，中间显示转圈与「正在加载…」；每张缩略图下载完成后淡入。这些在慢网络下尤其明显。

每张卡片自己带下载进度：缩略图由前端用 `fetch` 流式取回（所以能读到真实字节数与 `Content-Length`），
慢于一瞬时会显示**图片区底边的细条 + 中央百分比**；拿到的字节会随手包成图片显示出来，
所以是**从模糊逐渐变清晰**，而不是等整张下完才出现（靠的是给 JPEG 缩略图带 `interlace/1`，
服务端输出渐进式 JPEG，实测 12% 字节就能渲染出整张模糊图）。快到看不见的（HTTP 缓存命中、
几 KB 的小图）全程不显示进度层，免得每张都闪一下。灯箱里的大图走同一套，百分比显示在文件名上方。
拿不到 CORS、或服务端不认裁切参数（比如 GIF）时，会自动退回原图或退回浏览器的原生加载，卡片行为与以前一致。

### 公开画廊（游客可见）

- 想公开某张图片：在「我的相册」里把鼠标移到图片上，点右上角的**月亮图标**即可设为公开（变为**地球图标**、按钮高亮，文件名前也会出现 `pub` 徽标）；灯箱里也有「设为公开 / 取消公开」按钮
- 多选模式下可**批量**公开 / 取消公开
...
- **相册管理**：
  ...
  - **新建 / 重命名 / 移动**：都在自定义弹窗里完成，弹窗下方是一棵**可展开的相册目录树**，点任意相册即可把路径填进输入框（输入框里已有的相册名会保留在末尾，方便「移动到某相册下并改名」）
    ...
    - 会阻止移动到自身或其子相册
  - **删除相册**（相册卡片右上角的垃圾桶图标）：递归删除其中全部图片、子相册和空相册标记，确认框会写明将删除多少张图片
  - **多选模式**：勾选图片后批量删除、批量移动、批量公开
  - 单图删除在灯箱里
- **缩略图**：列表里一律用缩略图代替原图（游客和管理视图都一样），首屏流量通常能降一个数量级。**需要桶已开通 COS 图片处理**（数据万象基础图片处理，≤10TB/月 免费，等于不花钱），开关是 `config.js` 的 `thumb`，改它要重新推送部署。几点说明：
  - 边长由前端按**网格实际格子宽 × 屏幕像素比**算出（常见视口下约 200~450px，量化到 50px 台阶），所以 DPR1 的老显示器不会白下载 2 倍像素
  - 用的是 `crop/NxN/gravity/center` 裁出**居中正方形**，和格子的显示区域一比一对应 —— 比按长边缩放的 `thumbnail` 省 25%（4:3）到 44%（16:9）的流量。原图比 N 小时 crop 缩不动会**原样返回**（实测不会放大）
  - JPEG 再带 `interlace/1`（服务端输出渐进式 JPEG，用于「加载多少显示多少」）；**PNG 转 WebP**（`format/webp/quality/85`）—— 实测裁切后的 PNG 缩略图比 JPEG 重 6~8 倍（350px 要 99~182KB），转 WebP 后只要 9~22KB，逐像素差异平均 1.7~2.4（满值 255）。代价：WebP 截断时画不出东西，所以这些卡片不做增量显形（只有进度条）；另外若上传**真正带透明**的 PNG，缩略图会丢掉透明（原图与灯箱不受影响）
  - 单张 >32MB 或格式不支持（SVG / ICO 不走缩略图）时会自动退回原图，不会显示不出来，只是多一次请求
  - 灯箱点开看大图用的仍是**原图**，不受影响；原图也走同一套流式下载（多 MB 的时候会显示百分比），并在**鼠标停在某张卡片约 0.7 秒后开始预取原图**，所以点开通常是秒开 —— 不想让它预取就删掉 `renderGridItem` 里的 `mouseenter`/`mouseleave` 两行
...
- **成本**：GitHub Pages 与 Cloudflare Worker 免费，且**图片不经过 Worker**（前端直连 `album.ohtoai.top`），所以 Worker 的 10 万次/天额度几乎用不到。花钱的只有 COS：
  - 存储 0.118 元/GB/月，外网下行流量 0.5 元/GB，读写请求各 0.01 元/万次（上传的上行流量免费）
  - **流量费绝对主导**：500KB 的图被看一次，流量费 0.00024 元，请求费 0.000001 元，差约 244 倍
  - 所以最有效的省钱手段是开缩略图（基础图片处理 ≤10TB/月 免费）；个人用量通常每月一两元
  - `album.ohtoai.top` 若只是 COS 源站自定义域名（证书脚本只做了 `PutBucketDomainCertificate`），走的是上面 0.5 元/GB；只有在控制台另外开了 CDN 加速才按 CDN 价（0.21 元/GB 起，另加 COS 收的 CDN 回源流量费）
