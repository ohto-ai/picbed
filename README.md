...
- **相册管理**：
  ...
  - **「缩略图」开关**（工具栏最右）：列表里用缩略图代替原图，首屏流量通常能降一个数量级。**需要桶已开通 COS 图片处理**（数据万象基础图片处理，≤10TB/月 免费）。三点注意：
    - 缩略图边长由前端按**网格实际格子宽 × 屏幕像素比**算出（约 250~550px，量化到 50px 台阶），所以 DPR1 的老显示器不会白下载 2 倍像素；用的是 `thumbnail/NxN>`，长边不超过 N 且**只缩不放**（小图不会被放大）
...
| 想确认桶有没有开通图片处理 | 直接开一张图加参数：`https://<你的域名>/img/xxx.jpg?imageMogr2/thumbnail/500x500>`，返回缩小图即已开通 |
