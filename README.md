...
- **相册管理**：
  ...
  - 单图删除在灯箱里
  - **「缩略图」开关**（工具栏最右）：列表里用缩略图代替原图，首屏流量通常能降一个数量级。**需要桶已开通 COS 图片处理**（数据万象基础图片处理，≤10TB/月 免费）。三点注意：
    - 缩略图边长由前端按**网格实际格子宽 × 屏幕像素比**算出（常见视口下约 200~450px，量化到 50px 台阶），所以 DPR1 的老显示器不会白下载 2 倍像素
    ...
    - 工具栏只有「我的相册」视图可见，而 `config.js` 里的 `thumb` 才是**包含游客在内**的默认值；你自己勾的开关存在本机浏览器，优先级更高
